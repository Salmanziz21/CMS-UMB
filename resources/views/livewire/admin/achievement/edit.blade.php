...
<section class="w-full p-4">
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-xl font-bold text-gray-800 dark:text-gray-200">Edit Achievement</h1>
    </div>
    <x-form wire:submit.prevent="update" class="space-y-6">
        <flux:input wire:model.defer="title" type="text" label="Nama" required class="dark:text-gray-200" />
        <flux:textarea wire:model.defer="description" label="Bidang" required class="dark:text-gray-200" />
        ...
        <div class="space-y-2">
            ...
            <div class="flex flex-wrap gap-4">
                @if ($newImage)
                    <img src="{{ $newImage->temporaryUrl() }}" alt="Preview dokumentasi baru" class="h-10 w-10 rounded-xl object-cover border border-dashed border-emerald-300" />
                @elseif ($image)
                    <div class="flex items-center gap-2">
                        ...
                        @endif
                        <div class="h-10 w-10 bg-gray-200 dark:bg-gray-700 rounded-xl flex items-center justify-center border border-slate-200 {{ $imageExists ? 'hidden' : 'flex' }}">
