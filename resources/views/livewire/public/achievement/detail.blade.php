        ...
        <section id="home" class="relative overflow-hidden pt-32 pb-40">
            <div class="absolute inset-0">
                @if($achievement->image)
                    <img src="{{ asset('storage/' . $achievement->image) }}" alt="Dokumentasi {{ $achievement->title }}" class="h-full w-full object-cover">
                @else
            ...
            <div class="relative mx-auto flex max-w-6xl flex-col gap-12 px-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="max-w-3xl space-y-6">
                    <span class="inline-flex items-center gap-2 rounded-full bg-white px-5 py-2 text-xs font-semibold uppercase tracking-[0.35em] text-emerald-700 shadow-lg shadow-yellow-200/60">Karya & Prestasi</span>
                    ...
                    <div class="flex flex-wrap items-center gap-4 text-sm font-medium text-slate-700">
                        ...
                        <span class="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-4 py-2 text-emerald-700">
                            ...
                            {{ \Carbon\Carbon::parse($achievement->date)->translatedFormat('l, d F Y') }}
                        </span>
                    </div>
                    <div class="flex flex-wrap gap-3">
                        <a href="{{ route('home') }}#prestasi" class="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-emerald-400 via-emerald-300 to-emerald-400 px-6 py-3 text-sm font-semibold text-slate-950 shadow-lg shadow-emerald-500/40 transition hover:scale-[1.02]">
                ...
                @if($achievement->image)
                    <div class="w-full max-w-xl overflow-hidden rounded-[2.5rem] border border-white/60 bg-white/70 shadow-[0_35px_80px_-40px_rgba(16,185,129,0.4)]">
                        <img src="{{ asset('storage/' . $achievement->image) }}" alt="Dokumentasi {{ $achievement->title }}" class="h-[22rem] w-full object-cover">
                    </div>
                @endif
            </div>
        </section>
            ...
            <div class="mx-auto max-w-6xl space-y-10 px-6">
                <div class="grid gap-8 lg:grid-cols-[1.5fr_0.9fr]">
                    <article class="rounded-[2.5rem] border border-emerald-100 bg-white/95 p-10 shadow-[0_35px_80px_-40px_rgba(16,185,129,0.35)] ring-1 ring-emerald-500/10 backdrop-blur-sm">
                        <div class="space-y-4">
                            <p class="text-xs font-semibold uppercase tracking-[0.35em] text-emerald-600">Tentang Prestasi</p>
                            <h2 class="text-3xl font-semibold text-slate-900">{{ $achievement->title }}</h2>
                        ...
                        <div class="rounded-[2rem] border border-emerald-100 bg-white/90 p-8 shadow-[0_25px_60px_-40px_rgba(16,185,129,0.45)]">
                            ...
                            <ul class="mt-6 space-y-5 text-sm text-slate-700">
                                <li class="flex items-center gap-3">
                                    <span class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-emerald-50 text-emerald-600"><i class="fa-solid fa-tag"></i></span>
                                    {{ $achievement->category }}
                                </li>
                                <li class="flex items-center gap-3">
                                    ...
                                    {{ \Carbon\Carbon::parse($achievement->date)->isoFormat('dddd, DD MMMM YYYY') }}
                                </li>
                            </ul>
                        </div>
                    </aside>
